    ...
    public class UiSkinEditor : OdinMenuEditorWindow
    {
        ...
        protected override void OnBeginDrawEditors()
        {
            ...
            SirenixEditorGUI.BeginHorizontalToolbar();
            {
                ...
                if (SirenixEditorGUI.ToolbarButton("Documentation", true))
                {
                    Application.OpenURL(
...
//
        ...
        protected override OdinMenuTree BuildMenuTree()
        {
            ...
            CreateNewComponentData newComponentData = new CreateNewComponentData();
            tree.Add("Create New Skin Data", newComponentData);
        ...
        private class CreateNewPaletteData
        {
            ...
            public CreateNewPaletteData()
            {
                ...
                paletteData = CreateInstance<UiSkinPalette>();
        ...
        private class CreateNewComponentData
        {

            [InlineEditor(InlineEditorObjectFieldModes.Hidden)]
            public SimpleComponentSkinDataObject SkinData;

            [ValueDropdown("GetAllPalettes")] public string paletteName;

            private static IEnumerable GetAllPalettes()
            {
                return AssetDatabase.FindAssets("t:UiSkinPalette")
                    .Select(y => AssetDatabase.GUIDToAssetPath(y))
                    .Select(x => AssetDatabase.LoadAssetAtPath<UiSkinPalette>(x))
                    .Select(x => new ValueDropdownItem(x.displayName, x));
            }

            public CreateNewComponentData()
            {

                SkinData = CreateInstance<SimpleComponentSkinDataObject>();
                SkinData.name = "SKN_UI_Palette_";
            ...
            public void SaveSkinData()
            {
                ...
                SkinData.name = $"SKN_UI_{paletteName}_{SkinData.name}";

                if (!Directory.Exists($"Assets/Content/GUI/Styles/{paletteName}/"))
                    Directory.CreateDirectory($"Assets/Content/GUI/Styles/{paletteName}");

                AssetDatabase.CreateAsset(SkinData, $"Assets/Content/GUI/Styles/{paletteName}/{SkinData.name}.asset");
                AssetDatabase.SaveAssets();
                AssetDatabase.Refresh();
