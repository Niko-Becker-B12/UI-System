...
public static class UiDefaultPaletteGenerator
{
    ...
    public static void GenerateDefaultPaletteComponents(string paletteName)
    {
        ...
        foreach (var file in d.GetFiles("*.json"))
        {
            ...
            ComponentSkinDataObject componentSkinData = ComponentSkinDataObject.CreateInstance<ComponentSkinDataObject>();
            JsonUtility.FromJsonOverwrite(fileData, componentSkinData);
            

            
            AssetDatabase.CreateAsset(componentSkinData, $"Assets/Content/GUI/Styles/{paletteName}/{newName}.asset");
    ...
    public static void Save()
    {
        ...
        foreach (var subDir in subFolders)
        {
            ...
            foreach (var file in d.GetFiles("*.asset"))
            {
                ...
                ComponentSkinDataObject dataObject = AssetDatabase.LoadAssetAtPath($"{subDir}/{file.Name}", typeof(ComponentSkinDataObject)) as ComponentSkinDataObject;
